...
function playFullGame(state: GameState, maxMoves = 500): GameState {
  ...
  while (s.phase !== 'gameover' && moves < maxMoves) {
    if (s.awaiting_capture) {
      ...
      const mv = aiChoose(s, 'hard', { timeLimitMs: 80 });
      if (!mv) break;
...
describe('AI correctness', () => {
  it('aiChoose returns a valid move during placement', () => {
    ...
    const mv = aiChoose(s, 'medium', { timeLimitMs: 80 });
  ...
  it('aiChoose prefers mill-forming moves when available (easy)', () => {
    ...
    let foundMill = false;
    for (let i = 0; i < 50; i++) {
      const mv = aiChoose(state, 'medium', { timeLimitMs: 80 });
...
describe('AI smoke tests', () => {
  it('hard AI blocks a direct 1-ply mill threat when no better move exists', () => {
    // Set up: O (hard AI) must block X from completing a mill at point 2.
    // Opening book is bypassed by setting O as having already placed >= 4 pieces.
    const s = createInitialState();
    const board = Array(24).fill(null);
    ...
    const state: GameState = {
      ...
      turn: 'O',
      phase: 'placement',
      pieces_in_hand: { X: 7, O: 5 },
      pieces_on_board: { X: 2, O: 2 },
    };
    const mv = aiChoose(state, 'hard', { timeLimitMs: 120 });
    expect(mv).not.toBeNull();
    expect(mv!.type).toBe('place');
    expect(mv!.to).toBe(2);
  });
